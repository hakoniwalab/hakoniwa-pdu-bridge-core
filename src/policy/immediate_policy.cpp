...
bool ImmediatePolicy::should_transfer(const std::shared_ptr<hakoniwa::time_source::ITimeSource>& /* time_source */) {
    // For immediate policy, the answer is always yes when checked upon PDU update.
    return true;
}

void ImmediatePolicy::on_transferred(const std::shared_ptr<hakoniwa::time_source::ITimeSource>& /* time_source */) {
